...
fn regression_type_annotation_bug() {
    ...
    assert_eq!(
        5,
        interpret(
            "let add ~ fn(int, int) -> int = fn(x ~ int, y ~ int) -> int { x + y }; add(2, 3)"
        )
    );
    ...
    assert_eq!(
        25,
        interpret("let square ~ fn(int) -> int = fn(x ~ int) -> int { x * x }; square(5)")
    );
    assert_eq!(
        0,
        interpret("let is_false ~ fn() -> bool = fn() -> bool { false }; is_false()")
    );
}
...
fn regression_nested_scoping_variable_bug() {
    ...
    assert_eq!(
        15,
        interpret("let outer = 10; let inner = 5; outer + inner")
    );
...
fn regression_prevent_function_regressions() {
    ...
    assert_eq!(
        8,
        interpret("let add = fn(x ~ int, y ~ int) -> int { x + y }; add(3, 5)")
    );
    assert_eq!(
        25,
    ...
    assert_eq!(
        1,
        interpret("let not_fn = fn(x ~ bool) -> bool { false if x else true }; not_fn(false)")
    );
