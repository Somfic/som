...
fn test_function_call_errors() {
    ...
    assert!(expect_error("let f = fn(x ~ int, y ~ int) -> int { x + y }; f(1);"));
...
fn test_division_by_zero() {
    // Note: These might pass if division by zero is handled at runtime
    // Depending on implementation, these might be runtime errors
...
fn test_recursive_function_errors() {
    ...
    assert!(expect_error("let fact = fn(n ~ int) -> int { if n <= 1 else n * fact(n - 1) };"));
}
...
fn test_error_types() {
    ...
    assert_eq!(get_error_type("unknown_var"), Some("TypeCheckerError".to_string()));
