...
fn test_variable_declaration_variations() {
    ...
    assert_eq!(
        36,
        interpret("let x = 1; let y = 2; let z = 3; (x + y + z) * (x + y + z)")
    ); // (1+2+3)*(1+2+3) = 6*6 = 36
...
fn test_variable_scoping_with_functions() {
    ...
    assert_eq!(
        10,
        interpret("let x = 5; let f = fn(x ~ int) -> int { x }; f(10)")
    ); // Parameter x shadows outer x
    assert_eq!(
        20,
        interpret("let y = 15; let g = fn(z ~ int) -> int { z + 5 }; g(y)")
    ); // Function uses parameter, outer y passed as argument

    // Function local variable scoping
    assert_eq!(
        7,
        interpret("let x = 3; let f = fn() -> int { let x = 7; x }; f()")
    ); // Local x shadows outer x
    assert_eq!(
        3,
        interpret("let x = 3; let f = fn() -> int { let y = 7; x }; f()")
    ); // Function accesses outer x

    // Function accessing outer variables
    assert_eq!(
        13,
        interpret("let a = 10; let f = fn(b ~ int) -> int { a + b }; f(3)")
    ); // Function accesses outer variable
}
...
fn test_variable_initialization_with_variables() {
    ...
    assert_eq!(
        20,
        interpret("let x = 2; let y = x * x; let z = y * y; let w = z + y; w")
    ); // 2^2 = 4, 4^2 = 16, 16 + 4 = 20
}
...
fn test_variable_type_consistency() {
    ...
    assert_eq!(
        1,
        interpret("let flag = true; flag = false; flag = true; flag")
    );
...
fn test_variable_in_function_contexts() {
    ...
    assert_eq!(
        15,
        interpret(
            "let add = fn(x ~ int, y ~ int) -> int { x + y }; let a = 7; let b = 8; add(a, b)"
        )
    );
    assert_eq!(
        40,
        interpret("let square = fn(x ~ int) -> int { x * x }; let n = 6; square(n) + 4")
    );

    // Variables assigned from function returns
    assert_eq!(
        14,
        interpret("let double = fn(x ~ int) -> int { x * 2 }; let result = double(7); result")
    );
    assert_eq!(
        25,
        interpret("let get_five = fn() -> int { 5 }; let val = get_five(); val * val")
    );
    ...
    assert_eq!(
        15,
        interpret(
            "let multiplier = 3; let scale = fn(x ~ int) -> int { x * multiplier }; scale(5)"
        )
    );
}
...
fn test_variable_assignment_chains() {
    ...
    assert_eq!(
        10,
        interpret("let a = 1; let b = 2; let c = 3; c = 10; b = c; a = b; a")
    );
    assert_eq!(
        100,
        interpret("let x = 1; let y = 2; let z = 3; z = 100; y = z; x = y; x")
    );
}
