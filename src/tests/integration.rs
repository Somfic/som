...
fn integration_calculator_functions() {
    ...
    let program = r#"
        ...
        let square = fn(n ~ int) -> int { multiply(n, n) };
...
fn integration_complex_conditionals() {
    ...
    let program = r#"
        let max = fn(a ~ int, b ~ int) -> int { a if a > b else b };
        let min = fn(a ~ int, b ~ int) -> int { a if a < b else b };
    ...
    let simple_program = r#"
        let choose = fn(flag ~ bool, a ~ int, b ~ int) -> int { a if flag else b };
        let x = 10;
        let y = 5;
...
fn integration_function_composition() {
    ...
    let program = r#"
        ...
        let compose = fn(x ~ int) -> int { double(add_one(x)) };
...
fn integration_complex_arithmetic() {
    ...
    let program = r#"
        let calculate = fn(a ~ int, b ~ int, c ~ int) -> int {
            let step1 = a * b;
            let step2 = step1 + c;
            let step3 = step2 - a;
            step3
...
fn integration_recursive_style() {
    ...
    let program = r#"
        let factorial_step = fn(n ~ int, acc ~ int) -> int {
            acc * n
        };
        
        let compute_factorial_4 = fn() -> int {
...
fn integration_mixed_types() {
    ...
    let program = r#"
        let process = fn(num ~ int, flag ~ bool) -> int {
            let doubled = num * 2;
