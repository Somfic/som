...
fn test_function_definition_variations() {
    ...
    assert_eq!(
        8,
        interpret("let f2 = fn(x ~ int, y ~ int) -> int { x + y }; f2(3, 5)")
    );
    assert_eq!(
        15,
        interpret("let f3 = fn(a ~ int, b ~ int, c ~ int) -> int { a + b + c }; f3(2, 5, 8)")
    );
    ...
    assert_eq!(
        0,
        interpret("let f_bool_false = fn() -> bool { false }; f_bool_false()")
    );
...
fn test_function_parameter_types() {
    ...
    assert_eq!(
        10,
        interpret("let f = fn(flag ~ bool) -> int { 10 if flag else 5 }; f(true)")
    );
    assert_eq!(
        5,
        interpret("let f = fn(flag ~ bool) -> int { 10 if flag else 5 }; f(false)")
    );
    ...
    assert_eq!(
        7,
        interpret("let f = fn(x ~ int, flag ~ bool) -> int { x * 2 if flag else x }; f(7, false)")
    );

    // Functions returning booleans
    assert_eq!(
        1,
        interpret("let is_positive = fn(x ~ int) -> bool { x > 0 }; is_positive(5)")
    );
    assert_eq!(
        0,
        interpret("let is_positive = fn(x ~ int) -> bool { x > 0 }; is_positive(-3)")
    );
}
...
fn test_function_body_complexity() {
    ...
    assert_eq!(
        10,
        interpret("let f = fn(x ~ int) -> int { let y = x * 2; y }; f(5)")
    );
    ...
    assert_eq!(
        10,
        interpret("let abs = fn(x ~ int) -> int { x if x >= 0 else -x }; abs(10)")
    );
    assert_eq!(
        5,
        interpret("let abs = fn(x ~ int) -> int { x if x >= 0 else -x }; abs(-5)")
    );
...
fn test_function_calls_with_expressions() {
    ...
    assert_eq!(
        7,
        interpret("let add = fn(a ~ int, b ~ int) -> int { a + b }; add(2 + 1, 3 + 1)")
    );
    assert_eq!(
        20,
        interpret("let multiply = fn(x ~ int, y ~ int) -> int { x * y }; multiply(2 * 2, 6 - 1)")
    );
    ...
    assert_eq!(
        15,
        interpret(
            "let add = fn(a ~ int, b ~ int) -> int { a + b }; let x = 7; let y = 8; add(x, y)"
        )
    );
    assert_eq!(
        42,
...
fn test_function_calls_as_expressions() {
    ...
    assert_eq!(
        100,
        interpret("let get_true = fn() -> bool { true }; 100 if get_true() else 0")
    );
    assert_eq!(
        0,
        interpret("let get_false = fn() -> bool { false }; 100 if get_false() else 0")
    );
...
fn test_functions_with_scoping() {
    ...
    assert_eq!(
        20,
        interpret("let x = 5; let f = fn(x ~ int) -> int { x * 2 }; f(10)")
    ); // Parameter x shadows outer x

    // Function local variable scoping
    assert_eq!(
        5,
        interpret("let x = 10; let f = fn() -> int { let x = 5; x }; f()")
    ); // Local x shadows outer x
    assert_eq!(
        10,
        interpret("let x = 10; let f = fn() -> int { let y = 5; x }; f()")
    ); // Function accesses outer x

    // Block scoping within functions
    assert_eq!(
        15,
        interpret("let f = fn(x ~ int) -> int { let y = { let z = 5; z }; x + y }; f(10)")
    );
}
...
fn test_functions_returning_functions() {
    ...
    assert_eq!(
        20,
        interpret("let f = fn() -> int { 10 }; let g = fn() -> int { f() * 2 }; g()")
    );
}
...
fn test_recursive_functions() {
    ...
    assert_eq!(
        5,
        interpret("let f = fn(x ~ int) -> int { x }; let g = fn(y ~ int) -> int { f(y) }; g(5)")
    );
}
...
fn test_function_edge_cases() {
    ...
    assert_eq!(
        100,
        interpret("let const_100 = fn(x ~ int) -> int { 100 }; const_100(999)")
    );
    ...
    assert_eq!(
        728,
        interpret(
            "let cube_sum = fn(x ~ int, y ~ int) -> int { x * x * x + y * y * y }; cube_sum(6, 8)"
        )
    ); // 6^3 + 8^3 = 216 + 512 = 728
    assert_eq!(
        728,
...
fn test_function_assignments() {
    ...
    //     6,
    //     interpret(
    //         "let add = fn(x ~ int, y ~ int) -> int { x + y }; let my_func = add; my_func(2, 4)"
    //     )
    // );
...
fn test_functions_with_type_annotations() {
    ...
    assert_eq!(
        15,
        interpret(
            "let add ~ fn(int, int) -> int = fn(x ~ int, y ~ int) -> int { x + y }; add(7, 8)"
        )
    );
    assert_eq!(
        1,
        interpret("let is_pos ~ fn(int) -> bool = fn(x ~ int) -> bool { x > 0 }; is_pos(5)")
    );
...
fn test_function_call_chains() {
    ...
    assert_eq!(
        80,
        interpret("let double = fn(x ~ int) -> int { x * 2 }; double(double(double(10)))")
    );
