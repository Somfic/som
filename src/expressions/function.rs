...
pub fn compile(
    ...
    compile_with_optional_name(compiler, expression, env, None)
}

/// Compile a function, optionally pre-registering it for recursion
pub fn compile_with_optional_name(
    compiler: &mut Compiler,
    expression: &TypedExpression,
    env: &mut CompileEnvironment,
    ...
    // Analyze what variables this function captures from the environment
    let captured_vars = crate::lowering::captures::analyze_captures(&value, env);
    ...
    // If this is a named function with no captures, pre-register it for recursion
